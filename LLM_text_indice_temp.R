# LLM_text avec indicateur de sentiment #

# Description : Remplacer le niveau de confiance de chaque prévision par un indicateur de confiance
...
if (english == 1) {
  ...
  prompt_template <- function(type, d, q_trim, y_prev) {
    ...
    paste0(
      ...
      " and a score assessing between -4 and +4 assessing the performance of French economic activity. -4 means significantly contracting activity and +4 means a significantly expanding activity.",
  ...
  prompt_template <- function(type, d, q_trim, y_prev) {
    ...
    paste0(
      ...
      " et un score compris entre -4 et +4 décrivant l'activité économique française. -4 décrit une contraction significative de l'activité et +4 décrit une activité en expansion significative.",
...
forecast_score_pattern <- "([+-]?\\d+\\.?\\d*)\\s*\\(\\s*([+-]?\\d+)\\s*\\)"
...
for (dt in as.Date(dates$`Date Prevision`)) {
  ...
  prompt_text <- prompt_template(
    "BDF", current_date, trimestre_index,
    year_prev
  )
  ...
  parsed_list <- lapply(histoires, function(txt) {
    if (is.null(txt) || length(txt) == 0) {
      return(list(forecast = NA_real_, score = NA_integer_, raw = NA_character_))
    }
    m <- regmatches(txt, regexec(forecast_score_pattern, txt))
    if (length(m[[1]]) >= 3) {
      list(forecast = as.numeric(m[[1]][2]), score = as.integer(m[[1]][3]), raw = txt)
    } else {
      list(forecast = NA_real_, score = NA_integer_, raw = txt)
    }
  })
  ...
  for (i in seq_len(n_repro)) {
    ...
    df_bdf[[paste0("score_", i)]] <- parsed_list[[i]]$score
...
for (dt in as.Date(df_date$`Date Prevision`)) {
  ...
  year_prev <- if (mois_index == 1 && trimestre_index == 4) year_current - 1 else year_current
  prompt_text <- prompt_template(
    "INSEE", current_date, trimestre_index,
    year_prev
  )
  ...
  for (i in seq_len(n_repro)) {
    ...
    df_insee[[paste0("score_", i)]] <- parsed_list[[i]]$score
