...
source("Econ_metrics_analysis.R")
...
MODELS_STD <- c("BDF_txt", "INSEE_txt", "ALL_txt", "all_txt", 
                "ECO_ALL", "ECO_BDF", "ECO_INSEE")
...
print(ggplot(df_ranked, aes(x = Metrique, y = Model, fill = Rang)) +
...
metrics_llm_global <- read_xlsx("metrics_covid.xlsx") |>
...
df_PIB_clean <- read_xlsx("Data_BDF_INSEE.xlsx", sheet = "trimestriel") |>
  mutate(
    Date = as.Date(dates), forecast_year = year(Date), Month = month(Date),
...
print(ggplot(serie_temp, aes(y = Value, x = as.Date(Date.x), color = Line_Type)) +
  ...
  geom_rect(
    xmin = as.Date("2020-01-01"), xmax = as.Date("2022-01-01"),
...
#################################################################################
# LLM pur vs ECO pur vs Mix
##################################################################################


# Choisir selon covid ou non la bon bind à sélectionner
metrics_to_plot <- bind_rows(
  metrics_final_global,
  econ_metrics_table
)

metrics_to_plot <- bind_rows(
  metrics_final_nocovid,
  econ_metrics_table
)

# Modèles voulus :
target_models_list <- c(
  "BDF_txt", "INSEE_txt", # LLM txt
  "Sentiment BDF", "Sentiment INSEE", # LLM sentiment
  "Climat BDF (All)", "Climat INSEE (All)" # ECO
)

final_selection <- metrics_to_plot |>
  filter(Model %in% target_models_list)


# Appel de la fonction selon dummy covid
if (COVID == 0) { # à paramétrer depuis econ_metrics_analysis
  plot_ranking_rmse_sent(
    metrics_data = final_selection,
    title_plot = "Model ranking : LLM vs Econometrics vs Mixed models ",
