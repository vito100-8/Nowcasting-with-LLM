...
df_BDF_text_long <- df_BDF_text |>
  rowwise() |>
  mutate(Date = Date,
         median_forecast = mean(c_across(starts_with("forecast_")), na.rm = TRUE),
         .keep = "none"
  ) |>
  ungroup() |>
  mutate(
...
metrics_recap_final <- bind_rows(
  metrics_BDF_text, metrics_BDF_noText, metrics_BDF_rolling_text, metrics_BDF_just_text,  metrics_BDF_all,
  metrics_INSEE_text, metrics_INSEE_noText, metrics_INSEE_rolling_text, metrics_INSEE_just_text,  metrics_INSEE_all
)
