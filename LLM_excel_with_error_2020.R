...
for (idx in seq_along(dates)) {
  ...
  df_temp$LLM_prev_error <- sapply(df_temp$dates, function(drow) {
    ...
    if (!is.null(errors_by_data_BDF[[key]])) {
      return(as.character(errors_by_data_BDF[[key]]))
    }
    ...
    if (length(pib_match) >= 1) {
      ...
      if (!is.null(errors_by_quarter[[qk]])) {
        return(paste0(na.omit(as.character(errors_by_quarter[[qk]])), collapse = ";"))
      }
    }
    NA_character_
  })
...
for (idx in seq_along(dates)) {
  ...
  df_temp$LLM_prev_error <- sapply(df_temp$dates, function(drow) {
    key <- as.character(drow)
    if (!is.null(errors_by_data_INSEE[[key]])) {
      return(as.character(errors_by_data_INSEE[[key]]))
    } # <-- MODIFIÉ POUR INSEE
