...
detect_type <- function(s) {
  ...
  if (stringr::str_detect(txt, pattern_for(tokens_emi))) {
    return("EMI")
  }
  if (stringr::str_detect(txt, pattern_for(tokens_bat))) {
    return("BAT")
  }
  if (stringr::str_detect(txt, pattern_for(tokens_ser))) {
    return("SER")
  }
...
extract_type_and_date <- function(filename) {
  ...
  if (!all(is.na(m6))) {
    s6 <- m6[1, 2]
    # YYYYMM
    ya <- as.integer(substr(s6, 1, 4))
    ma <- as.integer(substr(s6, 5, 6))
    if (!is.na(ya) && !is.na(ma) && is_valid_month_year(ma, ya)) {
      return(return_if_valid(ma, ya))
    }
    # MMYY
    mb <- as.integer(substr(s6, 3, 4))
    yb <- expand_two_digit_year(substr(s6, 5, 6))
    if (!is.na(mb) && !is.na(yb) && is_valid_month_year(mb, yb)) {
      return(return_if_valid(mb, yb))
    }
    # MMYYYY
    mc <- as.integer(substr(s6, 1, 2))
    yc <- as.integer(substr(s6, 3, 6))
  ...
  months_alt <- c(
    "janv", "jan", "fev", "fevrier", "mar", "mars", "avr", "avril", "mai",
      ...
      if (!dry_run) {
        if (file.exists(newpath)) {
          newpath <- file.path(
            dirname(f),
            paste0(
              tools::file_path_sans_ext(newname),
